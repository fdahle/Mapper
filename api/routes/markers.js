...
router.post('/', (req, res) => {
  const { lat, lng, label, description, visited_at, planned_at, color, image_url, address, country, rating, is_favorite, use_coords, external_url, source, category_ids, collection_ids, collection_positions, person_ids } = req.body
  ...
  const { lastInsertRowid } = db
    .prepare("INSERT INTO markers (lat, lng, label, description, visited_at, planned_at, color, image_url, address, country, rating, is_favorite, use_coords, external_url, source, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))")
    .run(lat, lng, label || null, description || null, visited_at || null, planned_at || null, color || null, image_url || null, address || null, country || null, rating ?? null, is_favorite ? 1 : 0, use_coords ? 1 : 0, external_url || null, source || 'manual')
...
router.put('/:id', (req, res) => {
  ...
  const { label, description, visited_at, planned_at, color, image_url, address, country, rating, is_favorite, use_coords, external_url, category_ids, collection_ids, collection_positions, person_ids } = req.body
  ...
  db.prepare(
    "UPDATE markers SET lat=?, lng=?, label=?, description=?, visited_at=?, planned_at=?, color=?, image_url=?, address=?, country=?, rating=?, is_favorite=?, use_coords=?, external_url=?, updated_at=datetime('now') WHERE id=?"
  ).run(lat, lng, label ?? null, description ?? null, visited_at ?? null, planned_at ?? null, color ?? null, image_url ?? null, address ?? null, country ?? null, rating ?? null, is_favorite != null ? (is_favorite ? 1 : 0) : (existing.is_favorite ?? 0), use_coords != null ? (use_coords ? 1 : 0) : (existing.use_coords ?? 0), external_url ?? null, id)
...
router.patch('/:id/country', (req, res) => {
  ...
  if (!existing) return res.status(404).json({ error: 'Not found' })
  db.prepare("UPDATE markers SET country = ?, updated_at = datetime('now') WHERE id = ?").run(req.body.country ?? null, req.params.id)
  res.json({ ok: true })
})
